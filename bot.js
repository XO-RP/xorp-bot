...
const BOT_CONFIG = {
  ...
  'sync': false,
  'restWsBridgeTimeout': 9000, // check these
  'restTimeOffset': 300,
  'disabledEvents': [
    'CHANNEL_PINS_UPDATE',
    'TYPING_START'
...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    let dot = TICK_N % 1 === 0 ? 'XO' : 'Roleplay';
    let embed = new Discord.RichEmbed()
    .setAuthor("XO Roleplay Status", "https://cdn.discordapp.com/app-icons/725055754624565370/d11b747ed7ba0a58b4d3a7fe8c09f9f4.png?size=64" )
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ XO OP' : '⚫ XO RP')
    .setTimestamp(new Date())
    .setDescription('**Website** https://xorp.life')
    //.addField('\n\u200b\nconnect play.xorp.life:30120\n\u200b\n')
   // if (STATUS !== undefined)
   // {
  ...
  const offline = function() {
    ...
    .setColor(0xff0000)
    .addField('Server Status',':infinity: Restarting',true)
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        let embed = UpdateEmbed()
        .addField('Server Status',':ballot_box_with_check:   Online',true)
  ...
  bot.on('ready',() => {
    ...
    // bot.user.setGame('Roofstad', 'https://www.xorp.life/');
    bot.user.setActivity('XO RP',{'url':'https://www.xorp.life/','type':'PLAYING'});
    ...
    if (!message.author.bot) {
      if (message.member) {
        ...
        if (message.channel.id === BUG_CHANNEL) {
          ...
          .setColor(0x2894C2)
          .setTitle('Bug Report')
          .setDescription('sent to staff-team!')
          .setTimestamp(new Date());
